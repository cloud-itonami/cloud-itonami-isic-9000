(ns entertainment.operation
  "OperationActor -- one creative/arts/entertainment operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (EntertainmentOps-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Content and
  Booking Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one creative/arts/entertainment operation (intake ->
  ...
  decision to a human operator (the rights-holder/producer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:production/release` ALWAYS reaches this node when the
  governor is clean -- see `entertainment.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [entertainment.entertainmentopsllm :as entertainmentopsllm]
            [entertainment.governor :as governor]
...
(defn build
  ...
  `entertainment.store/Store`).
  opts:
    :advisor      -- an `entertainment.entertainmentopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (entertainmentopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EntertainmentOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (entertainmentopsllm/-advise advisor store request)]
            {:proposal p :audit [(entertainmentopsllm/trace request p)]})))

      ;; Content and Booking Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
